...
import { useInstallPrompt } from "@prodcontrol/hooks/useInstallPrompt";
import { useIsAdmin } from "@prodcontrol/hooks/useIsAdmin";

const navItems = [
  { to: "/prodcontrol", label: "Dashboard", icon: BarChart3, adminOnly: false },
  { to: "/prodcontrol/nova-observacao", label: "Nova Observação", icon: Plus, adminOnly: true },
  { to: "/prodcontrol/registros", label: "Registros", icon: ClipboardList, adminOnly: false },
  { to: "/prodcontrol/relatorios", label: "Relatórios", icon: FileBarChart, adminOnly: false },
];

const cadastroItems = [
  { to: "/prodcontrol/cadastro/obras", label: "Obras", icon: Building2 },
  { to: "/prodcontrol/cadastro/rotas", label: "Rotas", icon: Route },
  { to: "/prodcontrol/cadastro/especialidades", label: "Especialidades", icon: Wrench },
  { to: "/prodcontrol/cadastro/categorias", label: "Categorias", icon: Tag },
];
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-64 bg-sidebar flex flex-col border-r border-sidebar-border z-50">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        ...
        {isAdmin && (
          <NavLink to="/prodcontrol/auditoria" onClick={handleClick} className={`sidebar-item ${location.pathname === "/prodcontrol/auditoria" ? "sidebar-item-active" : ""}`}>
        ...
        {isAdmin && (
          <NavLink to="/prodcontrol/aprovacoes" onClick={handleClick} className={`sidebar-item ${location.pathname === "/prodcontrol/aprovacoes" ? "sidebar-item-active" : ""}`}>
        ...
        {isAdmin && (
          <>
            ...
            <NavLink to="/prodcontrol/configuracoes" onClick={handleClick} className={`sidebar-item ${location.pathname === "/prodcontrol/configuracoes" ? "sidebar-item-active" : ""}`}>
