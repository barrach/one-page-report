...
import { useIsMobile } from "@prodcontrol/hooks/use-mobile";
...
import StatCard, { Users, Clock, BarChart3, AlertTriangle } from "@prodcontrol/components/StatCard";
import {
...
import { Badge } from "@prodcontrol/components/ui/badge";
...
import { ChartZoomDialog, ZoomButton } from "@prodcontrol/components/ChartZoomDialog";
...
import { normalizeDescriptionName } from "@prodcontrol/lib/categoryNormalization";
import { normalizeTime } from "@prodcontrol/lib/chartConstants";
import { computeHourlyAdjustedPercentages, computeHHMedioDia, getRecordHHWithContext, normalizeToHundred } from "@prodcontrol/lib/hourlyAverageCalc";
import { applyManualAdjustments } from "@prodcontrol/lib/manualAdjustments";
import { applyDbWeighting } from "@prodcontrol/lib/manualAdjustments";
import { LegendTooltip } from "@prodcontrol/components/LegendTooltip";
import NpeTraceabilityModal from "@prodcontrol/components/NpeTraceabilityModal";
...
export default function Dashboard() {
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      const { captureAllCharts } = await import("@prodcontrol/lib/chartCapture");
      ...
      // 3) Generate PDF
      const { generatePDFReport } = await import("@prodcontrol/lib/pdfReport");
  ...
  const exportToPPTX = async () => {
    ...
    try {
      ...
      // 3) Generate PPTX
      const { generatePPTXReport } = await import("@prodcontrol/lib/pptxReport");
      generatePPTXReport({
        periodo: aiStats.periodo,
        obra: aiStats.obra,
        totalAmostras: aiStats.totalAmostras,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {(() => {
          ...
          const getSpecProd = (recs: any[]) => {
            ...
            recs.forEach((r: any) => {
              const specName = (r.especialidades as any)?.nome || "Sem especialidade";
              if (!grouped[specName]) grouped[specName] = { trab: 0, plan: 0, total: 0 };
              const hh = getHH(r);
              const desc = canonicalDescription(r.descricao || "");
              grouped[specName].total += hh;
              if (desc === "Trabalhando") grouped[specName].trab += hh;
              else if (desc === "Planejando") grouped[specName].plan += hh;
            });
            const result: Record<string, number> = {};
            for (const [name, v] of Object.entries(grouped)) {
              result[name] = v.total > 0 ? ((v.trab + v.plan) / v.total) * 100 : 0;
            }
            return result;
          };
